...
def test_transformer_donnees_types_et_calculs(mock_payload_binance):
    ...
    assert list(df_resultat.columns) == ["id", "time", "price", "qty", "montant_usd", "isBuyerMaker"]
    
    # 2. Vérification des Types (La clé d'un ETL solide)
    assert pd.api.types.is_datetime64_any_dtype(df_resultat["time"]), "La colonne 'time' doit être un datetime"
    assert pd.api.types.is_float_dtype(df_resultat["price"]), "La colonne 'price' doit être convertie en float"
    assert pd.api.types.is_float_dtype(df_resultat["qty"]), "La colonne 'qty' doit être convertie en float"
    ...
    assert df_resultat.iloc[0]["time"] == pd.to_datetime(1672531200000, unit="ms"), "Erreur de conversion temporelle"
...
def test_ecriture_bdd_mock(mock_payload_binance):
    ...
    with moteur_mock.connect() as connection:
        resultat = connection.execute(sqlalchemy.text("SELECT COUNT(*) FROM table_alertes_baleines"))
